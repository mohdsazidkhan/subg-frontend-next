...
import { FaArrowLeft, FaChartLine, FaUsers, FaTrophy, FaMoneyBillWave } from 'react-icons/fa'
...
const DashboardAnalytics = () => {
  ...
  if (loading) {
    return (
      <AdminMobileAppWrapper title="Dashboard Analytics">
    ...
    <AdminMobileAppWrapper title="Dashboard Analytics">
      <div className="p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Quiz Completion Rate</h3>
            <div className="text-3xl font-bold text-blue-600 dark:text-blue-400">
              {analytics.quizCompletionRate}%
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Average completion rate</p>
          </div>

          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">User Retention</h3>
            <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">
              {analytics.userRetentionRate}%
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Monthly retention rate</p>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {analytics.topCategories?.map((category, index) => (
              <div key={category._id} className="flex items-center justify-between">
                ...
                <span className="text-gray-600 dark:text-gray-400">{category.quizCount} quizzes</span>
              </div>
            ))}
          </div>
        </div>
